...
exports.createDepartment = async ({ name, zp_id, code, name_mr }) => {
    ...
    try {
        result = await client.query(
            `INSERT INTO departments (name, code, name_mr) VALUES ($1, $2, $3)
        ON CONFLICT(code) DO UPDATE SET
        status=1, NAME=EXCLUDED.NAME, name_mr=EXCLUDED.name_mr
        RETURNING *`,
            [name, code, name_mr]
        );
        await client.query(`INSERT INTO zp_departments (zp_id, department_id) VALUES($1, $2) ON CONFLICT DO NOTHING`, [zp_id, result.rows[0].department_id]);
...
exports.getDepartmentHead = async ({ user_id }) => {
    ...
    JOIN zp_departments zd ON d.department_id = zd.department_id
    JOIN role_permissions rp ON rp.role_id = r.role_id
    JOIN permissions p ON p.permission_id = rp.permission_id

    WHERE r.name = 'dept_head' AND zd.zp_id=$1
